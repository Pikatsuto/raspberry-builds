...
# Install hostapd configuration files
echo "Installing hostapd configuration files..."
if [ -f /root/setupfiles/hostapd-5ghz.conf ]; then
    mkdir -p /etc/hostapd
    mv /root/setupfiles/hostapd-5ghz.conf /etc/hostapd/hostapd-5ghz.conf
    mv /root/setupfiles/hostapd-2.4ghz.conf /etc/hostapd/hostapd-2.4ghz.conf
    mv /root/setupfiles/hostapd-5ghz.service /etc/systemd/system/hostapd-5ghz.service
    mv /root/setupfiles/hostapd-2.4ghz.service /etc/systemd/system/hostapd-2.4ghz.service
    systemctl daemon-reload
    echo "  Hostapd configuration files installed"
else
...
# Note: Incus and Docker will be initialized on first boot by services-first-boot.sh
